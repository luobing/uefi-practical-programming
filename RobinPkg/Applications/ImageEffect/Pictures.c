...
//================================================  JPEG Begin ================================================
//Function Name: ShowBMP24True
//Input: fileName: jpeg file,24 bit true color
//       x,y:      position in display screen
//Output: EFI_SUCCESS: success
//        Other value: fail
...
//Function Name: Jpeg2BltBuffer
//Input: fileName: jpeg file,24 bit true color
//       GopBlt:   store blt buffer of jpeg
//       pixelWidth,pixelHeight: picture's width,height
//Output: EFI_SUCCESS: success
//        Other value: fail
EFI_STATUS Jpeg2BltBuffer(IN char *fileName, IN OUT VOID **GopBlt,OUT UINTN *pixelWidth,OUT UINTN *pixelHeight)
{
  EFI_GRAPHICS_OUTPUT_BLT_PIXEL *bltBuffer =NULL;
  void *jfif = NULL;
  BMP_FORJPG   bmp  = {0};
  ...
  *pixelHeight = bmp.height;
  // bltBuffer = AllocateZeroPool(*GopBltSize);
  bltBuffer     = AllocatePool (sizeof(EFI_GRAPHICS_OUTPUT_BLT_PIXEL)* (bmp.height) * (bmp.width));
  if(bltBuffer == NULL)
    return EFI_BUFFER_TOO_SMALL;

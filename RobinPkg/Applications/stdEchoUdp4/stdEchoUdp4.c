  ...
  IN char **Argv)
{
  ...
  char msgStr[1024];
  char recvStr[1024];
  socklen_t ServerIpLength;
  //1 get the server ip and port
  if(Argc != 3)
  {
    printf("UEFI UDP client. Usage: %a <ServerIP> <port>\n", Argv[0]);
    Status = EFI_ABORTED;
    return Status;
  }
  else
  ...
  while (1)
  {
    ...
    // sendto(socketID, msgStr, strlen(msgStr), 0, (struct sockaddr *)&ServerIp, ServerIpLength);

    recvLen = (int)recv(socketID, recvStr, sizeof(recvStr), 0);
    // recvLen = recvfrom(socketID, recvStr, sizeof(recvStr), 0,(struct sockaddr *)&ServerIp, &ServerIpLength);
    recvStr[recvLen] = '\0';
